...
test("generation pipeline returns one technical document and canonical model", async () => {
...
test("generation pipeline ignores legacy template and AI mode options", async () => {
  const zip = createZipBuffer([
    {
      fileName: "META-INF/MANIFEST.MF",
      content: "Bundle-Name: Demo\nBundle-Version: 1.0.0\nBundle-Vendor: SAP",
  ...
  assert.deepEqual(result.selectedTemplateIds, ["technical"]);
